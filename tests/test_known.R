...
highlim=rep(1.2,2)
#Test
swarm_state <- initialize_swarm(desired_values = ydat,param_len = 6,lowlim=lowlim,highlim=highlim,ineq_w = c(),eq_w=c(),config = swarm.config(num_cluster=3,deg=2))

for (itt in 1:100){
  tempcontrol = swarm.control(poly_w=0.2,stoch_w = 10)
