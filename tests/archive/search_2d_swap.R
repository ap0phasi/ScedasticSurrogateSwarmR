...
for (itt in 1:10){
  ...
  matplot(xdat,swarm_state$current_pout,type="l",col="grey")
  points(xdat,ydat)
  lines(xdat,surrogate_out,col="blue")
  lines(xdat,model_function(as.vector(swarm_state$poly_recs[1,]),ls),col="red")
}
print(paste("Number of function evals: ",dim(swarm_state$xpins)[1]))

#Swap into particle swarm
x.psave = swarm_state$xpins[(dim(swarm_state$xpins)[1]-50):dim(swarm_state$xpins)[1],]
centersaves = swarm_state$centersaves
polyouts = swarm_state$polyouts
swarm_state <- initialize_swarm(desired_values = ydat,param_len = 2,lowlim=lowlim,highlim=highlim,ineq_w = c(),eq_w=c(),config = swarm.config(swarm_size=dim(x.psave)[1],num_cluster=3,deg=2,x.p=x.psave))
swarm_state$centersaves=centersaves
swarm_state$polyouts=polyouts

for (itt in 1:10){
  tempcontrol = swarm.control(poly_w=0.2,stoch_w = 0.01)
  swarm_state <- step_swarm(desired_values = ydat,swarm_state,ineq_w = c(),eq_w=c(),config = swarm.config(swarm_size=dim(x.psave)[1],num_cluster=3,swarm.control = tempcontrol))
