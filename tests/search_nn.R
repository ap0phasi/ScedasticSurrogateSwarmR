...
genstate = F
for (itt in 1:5){
  tempconfig = search.config(gen = genstate,deg=1,search_samples = ls)
...
#usevec = (dim(search_state$xpins)[1]-400):dim(search_state$xpins)[1]
...
#Compare to BOBYA
result <- minqa::bobyqa(runif(3*ls,0,1), bobya_function, lower = lowlim, upper = highlim)
