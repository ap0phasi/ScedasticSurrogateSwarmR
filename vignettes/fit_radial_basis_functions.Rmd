...
# Scedastic Surrogate Swarm R Package - Fitting Radial Basis Functions
...
```{r}
swarm_state <- initialize_swarm(desired_values = ydat,param_len = 6,lowlim=lowlim,highlim=highlim,ineq_w = c(),eq_w = c(),config = swarm.config(swarm_size = 30,deg=2))

for (itt in 1:30){
  tempcontrol = swarm.control(poly_w=0.2,stoch_w = 0.2)
  swarm_state <- step_swarm(desired_values = ydat,swarm_state,ineq_w = c(),eq_w=c(),config = swarm.config(swarm_size=dim(swarm_state$x.p)[1],num_cluster=1,deg = 2,swarm.control = tempcontrol))
...
genstate = F
for (itt in 1:5){
  tempconfig = search.config(gen = genstate,deg=1,search_samples = 12,search_mag=0.2,revert_best = T)
...
for (itt in 1:20){
  tempcontrol = swarm.control(poly_w=0.2,stoch_w = 0.2)
  swarm_state <- step_swarm(desired_values = ydat,swarm_state,ineq_w = c(),eq_w=c(),config = swarm.config(swarm_size=length(usevec),num_cluster=1,deg=1,swarm.control = tempcontrol))
  par(oma=c(0, 0, 0, 5))
  matplot(xdat,swarm_state$pout,type="l",col="grey",xlab = NULL,ylab="value",ylim = c(0,1.2))
  points(xdat,ydat)
